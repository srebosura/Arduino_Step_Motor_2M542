...
const int loading_enable = 9;
const int loading_enable2 = 22;
...
void setup()   /*----( SETUP: RUNS ONCE )----*/
{
  ...
  pinMode(loading_enable,OUTPUT); //loading IR VCC
  pinMode(loading_enable2,OUTPUT);
...
if (digitalRead(start_buttonPin)==0)  { 
  ...
  digitalWrite(loading_enable, HIGH);
  digitalWrite(loading_enable2,HIGH);
...
void read_sensors() {
 //Interrupt Service Routine, read the state of the stop switch into a local variable: 
  ...
  Serial.println("Stop Interrupt called ");
  int read_vac_sens = digitalRead(vacuum_status);
  int read_brusher_stat = digitalRead(brusher_status);
 // int read_safety_stat = digitalRead(cover_safety_status); // not use as interrupt
  tone( piezoPin, 2300, 250);
  ...
  if (read_stop == LOW && run_status == 1 && stop_status == 2) {
          Serial.println("Reset 0");
          lcd.clear();
          resetFunc();
      }
  if (read_stop == LOW && run_status == 2) {
   ...
   if (read_stop == LOW && run_status == 4) {
          Serial.println("Reset 2");
          lcd.clear();
          load_status = 1;
      }

  if (read_stop == LOW && read_vac_sens == 0) {
          Serial.println("Vacuum Error");
          tone( piezoPin, 2000, 250);  
          error_msg = "< Vacuum Error >";
          //run_status = 2;
          //lastButtonPushed = 20;  
  ...
  if (read_stop == LOW && read_brusher_stat == 0) {
          Serial.println("<Brusher Error> ");
          tone( piezoPin, 2000, 250);  
          error_msg = "<Brusher Error> ";
          //run_status = 2;
         // lastButtonPushed = 20;   
         ...
         exit;
          ...
          Serial.println("Stop");
          //lastButtonPushed=7;
          stop_status = stop_status + 1;   
          auto_status = 0;
          //run_status = 0;
          delay(100);    
    //Serial.print("stop_status = ");Serial.println(stop_status); 
   // Serial.print("run_status = ");Serial.println(run_status);
    //Serial.print("alarm = ");Serial.println(ret_alarm);
    //Serial.print("load_status = ");Serial.println(load_status);
 }
...
void run_step_m2(int dir2, float rev2) {    
  ...
  digitalWrite(m2_step, LOW);
  time_delay2(motor_speed_m2);
  if (m2_extreme_pos == LOW && auto_status == 0) {
    //detachInterrupt(2);
   ...
   if (load_status == 1  && run_status == 4) {
      break;
      } 
   if (m2_extreme_pos == LOW && auto_status == 1) {
    //detachInterrupt(2);
    error_msg = "Magazines Empty!";
    //run_status=2;
    //lastButtonPushed = 20;
    magz_status = 1;
    break;   
...
int auto_mode (int cycle) {
  auto_status = 1;
  stop_status = 0; // reset stop button flag
  if (digitalRead(cover_safety_status) == LOW) {
      error_msg = "<Cover is Open >";
      run_status = 2;
      return 5;
  ...
  for (int y = 0; y <cycle; y++) {
   ...
   delay (250); //debounce delay to make sure noise is not read
    if (read_frame == 0 && digitalRead(vacuum_status) == LOW) {
      run_status = 2;
      return 4;
      }
    ...
    if (read_frame == 1) { //no frame activated
      ...
      Serial.print("auto_status = ");Serial.println(auto_status);
      delay (500);
     }
...
void load() { 
  ...
  delay(1000);
  digitalWrite(loading_enable, LOW);
  delay(1000);
  digitalWrite(loading_enable2,LOW);
  delay(1000);
  Serial.println("Loading Sensor Enable");
  lcd.setCursor(0, 1);
  ...
  digitalWrite(loading_enable2,HIGH);
  delay(1000);
  Serial.println("Loading Sensor Disable");
  lcd.clear();
  lcd.setCursor(0, 0);
