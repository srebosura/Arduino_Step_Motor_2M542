...
const int frame_sensor1 = 5; //frame detect sensor IR
...
float EEPROM_read_magz = 0.0f; // variable for magazine motor
...
float EEPROM_frame_width = 0.00; // variable for frame width

float push_delay = 0.0f;
float pusher_steps = 0.0f;
float magz_steps = 0.0f;
float pusher_FW_spd;
...
void setup()   /*----( SETUP: RUNS ONCE )----*/
{
  ...
  pinMode(frame_sensor1, INPUT); //active high
...
if (digitalRead(start_buttonPin)==0)  { 
   ...
   EEPROM.get(EE_addr3,EEPROM_read_magz);
   magz_steps = EEPROM_read_magz;
   ...
   EEPROM.get(EE_addr6,EEPROM_frame_width);
   frame_width = EEPROM_frame_width;
...
void main_menu() {
   ...
   switch (lastButtonPushed) {
    ...
    case 1:
      ...
      ret = auto_mode(magz_steps); //set the number of feeds
        if (ret == 1) {
          //lcd.clear();
          display_status_lcd(0,0,"Run Status = ",run_status);
      ...
      run_status = 0;
      display_status_lcd(0,0,"Run Status = ",run_status);
      display_lcd(0,1,"<Empty Magazine>");
      Serial.println("Magazine is Empty");
      digitalWrite(run_ledPin, LOW);
      lastButtonPushed=12;      
      delay(2000);         
      break;
    ...
    case 12:
      digitalWrite(run_ledPin, LOW);
      scroll(" <<< RELOAD >>>  ",13,1);
      display_lcd(0,0,"Magazines Empty!"); 
...
void read_sensors() {
 //Interrupt Service Routine, read the state of the stop switch into a local variable: 
  ...
  if (read_stop == LOW && run_status == 2) {
    ...
    display_lcd(0,1,"Ending Seaquence");
    digitalWrite(run_ledPin, LOW);
    Serial.print("run_status = ");
    Serial.println("Stop");
    lastButtonPushed=7;   
...
int auto_mode (int cycle) {
  ...
  for (int y = 0; y <cycle; y++) {
    ...
    motor_speed_m2 = 2;
    run_step_m2(1,frame_width);
   read_frame = digitalRead(frame_sensor1);
   delay (100);
...
void unload() {
  check_home();
  Serial.println("Unloading the Magazine.....");
  display_lcd(0,1,"Unloading magz..");
  motor_speed_m2 = 1;
  run_step_m2(1,magz_steps/2);
  Serial.print("..m2_unload_position"); 
  delay(1000); 
}

void load() {
  magz_count = magz_steps;
  load_level = 3;    
  Serial.println("Loading the Magazine.....");
  check_home();
  lcd.clear();
  lcd.setCursor(0, 1); 
  lcd.print("Loading magz....");
  magz_count = magz_count-(magz_steps*0.25);
  motor_speed_m2 = 1;
  ...
  lcd.print(load_level);     
   while (load_level != 0) {
        ...
        if (digitalRead(start_buttonPin)== 0) {
          ...
          lcd.setCursor(0, 1);   
          lcd.print("Loading magz....");
          run_step_m2(0,(magz_steps*0.25)/2);      
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("HIT  ENT to LOAD");
            lcd.setCursor(0, 1);   
...
void magz_forward() {
  if (digitalRead(motor2_extreme_sensor) == LOW) {
    ...
    Serial.println("Magz Backward ");
    display_lcd(0,1,"Magz Backward...");    
    motor_speed_m2 = 1;
    run_step_m2(1,frame_width);
    delay (250);
}
...
void set_magz_cap() {
  ...
  lcd.print("HIT  ENT to SAVE");
  lcd.setCursor(0, 1);
  lcd.print("Magazine Cap =");
  lcd.print(EEPROM_read_magz,0);
  magz_steps = EEPROM_read_magz;
  delay(2000);
...
void set_pusher_FW_speed() {
  ...
  while (lastButtonPushed==17) {
        ...
        if (digitalRead(select_buttonPin)== 0) {
          delay(150);
          pusher_FW_spd = pusher_FW_spd+0.25;
          tone( piezoPin, 2000, 150);
...
void set_pusher_BKD_speed() {
  ...
  while (lastButtonPushed==19) {
        ...
        if (digitalRead(select_buttonPin)== 0) {
          delay(150);
          pusher_BKD_spd = pusher_BKD_spd+0.25;
          tone( piezoPin, 2000, 150);
...
void set_frame_width() {
  ...
  lcd.print("HIT  ENT to SAVE");
  lcd.setCursor(0, 1);
  lcd.print("Frame Width=");
  lcd.print(EEPROM_frame_width,2);
  frame_width = EEPROM_frame_width;
  delay(2000);
