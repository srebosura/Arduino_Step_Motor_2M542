...
void run_step_m1(int dir1, long rev1) {    
  ...
  if (m1_home_status == LOW && run_status == 1) {
    error_msg = "<<M1 REF ERROR>>";
    run_status=2;
    lastButtonPushed = 20;
    main_menu();
   }  
...
void run_step_m2(int dir2, float rev2) {    
  ...
  if (m2_home_status == LOW && run_status == 1) {
    error_msg = "<<M2 REF ERROR>>";
    run_status=2;
    lastButtonPushed = 20;
    main_menu();
   } 
...
void load() {    
  ...
  magz_count=magz_count-(Frame_steps*0.5); //orig 0.25
  motor_speed_m2 = 1;
