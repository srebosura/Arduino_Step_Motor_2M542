...
void read_sensors() {
 ...
 // int read_safety_stat = digitalRead(cover_safety_status);
  tone( piezoPin, 2300, 250);
  ...
  if (read_stop == LOW && read_brusher_stat == 0 && run_status == 4 ) {
...
void load() { 
  run_status = 0;
  stop_status = 0; //clear stop_button flag   
  magz_count = 94;
  load_level = 3;    
  Serial.println("Loading the Frame.....");
  check_home();
  delay(1000);
  Serial.println("M1 to Loading position");
  display_lcd(0,0, "<  LOAD FRAME  >");
  display_lcd(0,1, "<Set LoadingPOS>");
  motor_speed_m1 = 1.75;
  run_step_m1(0,150);
  delay(500);
