...
const int frame_sensor = 5; //frame detect sensor IR
...
//individual frame sensor
const int frame_sensor1 = 23;
const int frame_sensor2 = 25;
const int frame_sensor3 = 27;
//vacuum control
const int vacuum1 = 22;
const int vacuum2 = 24;
const int vacuum3 = 26;
...
void setup()   /*----( SETUP: RUNS ONCE )----*/
{
  ...
  pinMode(frame_sensor, INPUT_PULLUP); //active high
  pinMode(frame_sensor1, INPUT_PULLUP);
  pinMode(frame_sensor2, INPUT_PULLUP);
  pinMode(frame_sensor3, INPUT_PULLUP);
  pinMode(vacuum1, OUTPUT);
  pinMode(vacuum2, OUTPUT);
  pinMode(vacuum3, OUTPUT);
...
int auto_mode (int cycle) {
  ...
  for (int y = 0; y <cycle; y++) {
    ...
    run_step_m2(1,frame_thick);
   read_frame = digitalRead(frame_sensor); //read_frame = 0; << to disable use this
...
void unload() {
  ...
  display_lcd(0,1,"Unloading Frame..");
  motor_speed_m2 = 1;
...
void load() {    
  ...
  magz_count = Frame_steps - (Frame_steps*0.25);
  motor_speed_m2 = 1;
  load_level=load_level-1;
  run_step_m2(1,magz_count*frame_thick*2);
  lcd.clear();
  lcd.setCursor(0, 0);
  ...
  lcd.print(load_level);
   while (load_level != 0) {
        ...
        if (digitalRead(start_buttonPin)== 0) {
          ...
          run_step_m2(0,Frame_steps*0.25*frame_thick*2);
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Hit ENT to Load");
            lcd.setCursor(0, 1);
...
void vacuum_control(int solenoid_num, int vacuum_state) {
 // vaccum_state 0-disable,1-enable
 // solenoid numbers are 1,2,4 and 7 indicates all 3 selected 
  if (solenoid_num = 1) {
    digitalWrite(vacuum1, vacuum_state);
  }
  if (solenoid_num = 2) {
    digitalWrite(vacuum2, vacuum_state);
  }
  if (solenoid_num = 4) {
    digitalWrite(vacuum3, vacuum_state);
  }
  if (solenoid_num = 3) {
    digitalWrite(vacuum1, vacuum_state);
    digitalWrite(vacuum2, vacuum_state);
...
int frame_check() {
  ...
  if (digitalRead(frame_sensor1)== 0) {
    delay (150);
    frame_stat = frame_stat + 1;
  }
  if (digitalRead(frame_sensor2)== 0) {
    delay (150);
    frame_stat = frame_stat + 2;
  }  
  if (digitalRead(frame_sensor3)== 0) {
     delay (150);
    frame_stat = frame_stat + 4;
  }
  return frame_stat; 
}
